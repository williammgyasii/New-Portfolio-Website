...
export default function AnimatedPillTags({
  ...
  return (
    <div
      className={cn(
        " w-full item-center justify-center text-neutral-100 antialiased",
        className
      )}
    >
      <button
        className={cn(
          "shimmer relative inline-flex items-center rounded-full p-[2px]  bg-gradient-to-r from-sky-300 via-sky-500 to-sky-900 [background-size:200%_100%] focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-300/60"
        )}
        aria-label="Live pill"
      >
        <span
          className={cn(
            "inline-flex items-center gap-2 rounded-full bg-sky-950 px-6 text-sky-200 py-2 text-sm",
            size === "small" && "px-3 py-1 text-xs",
            size === "large" && "px-8 py-3 text-lg font-semibold",
            size === "medium" && "px-5 py-2 text-sm"
          )}
        >
